...
IF (SELECT COUNT(*) FROM sys.objects WHERE [Name] = 'ufns_StringToPascalCase' AND [type_desc] = 'SQL_SCALAR_FUNCTION' AND [type] = 'FN') > 0
 ...
 BEGIN
	...
	--check if all characters in function provided string are alpha or spaces
		WHILE (@LoopIndex <= @LoopCeiling)
			BEGIN
				IF (ASCII(SUBSTRING(@OriginalString, @LoopIndex, 1)) NOT BETWEEN 97 AND 122)	--lower case alpha (only need to consider lower case ASCII values due to the LOWER function being called - above - on the @InputValue argument)
					AND ASCII(SUBSTRING(@OriginalString, @LoopIndex, 1)) <> 39	--allow apostrophes
						SET @AllAlphaFlag = 0
	...
	--If all character's in the argument passed into this function are alpha (or an apostrophe), loop through the string capitolizing the first letter in each word
		IF (@AllAlphaFlag = 1)	--confirm that there are no non-alpha characters being passed into this function
			BEGIN
